...
class JointDataset(LoadImagesAndLabels):  # for training
    ...
    def __init__(self,
        ...
        for ds, label_paths in self.label_files.items():
            ...
            for lp in label_paths:
                ...
                if len(lb.shape) < 2:  # 该图片的标签中的轨迹数<=2
                    track_max = lb[1]
                else:
                    track_max = np.max(lb[:, 1])
                if track_max > max_index:
                    max_index = track_max

            self.tid_num[ds] = max_index  # max_index + 1
        ...
        self.nID = int(last_index)  # int(last_index + 1)  # 所有子训练集的轨迹总数
    ...
    def __getitem__(self, files_index):
        ...
        hm = np.zeros((num_classes, output_h, output_w),  # C×H×W
